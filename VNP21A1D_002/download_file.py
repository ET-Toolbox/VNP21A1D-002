...
def download_file(
    ...
    Raises:
        VNP21A1DDownloadFailed: If the download fails.
    """
    # If filename is not provided, construct it using the granule_directory and the basename of the URL
    if filename is None:
        filename = join(parent_directory, posixpath.basename(URL))
    ...
    if exists(expand_filename(filename)) and getsize(expand_filename(filename)) == 0:
        logger.warning(f"removing zero-size corrupted VNP21A1D file: {filename}")
        os.remove(filename)
    ...
    if not exists(partial_filename):
        raise VNP21A1DDownloadFailed(f"unable to download URL: {URL}")
    # Check if the partial file is zero-size, remove it and raise an exception
        ...
        logger.warning(f"removing zero-size corrupted VNP21A1D file: {partial_filename}")
    ...
    if not exists(expand_filename(filename)):
        raise VNP21A1DDownloadFailed(f"failed to download file: {filename}")
